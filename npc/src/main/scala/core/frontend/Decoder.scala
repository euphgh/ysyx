...
object RV64I extends DecodeUtils {
  ...
  val rv64i: InstrPat = Array(
    ...
    JAL -> List(NumSrcReg.zero, HasDstReg.yes, HFuType.bru, BranchType.jal),
    JALR -> List(NumSrcReg.one, HasDstReg.yes, HFuType.bru, BranchType.jalr),
    BEQ -> List(NumSrcReg.two, HasDstReg.no, HFuType.bru, BranchType.beq),
    BNE -> List(NumSrcReg.two, HasDstReg.no, HFuType.bru, BranchType.bne),
    BLT -> List(NumSrcReg.two, HasDstReg.no, HFuType.bru, BranchType.blt),
    BGE -> List(NumSrcReg.two, HasDstReg.no, HFuType.bru, BranchType.bge),
    BLTU -> List(NumSrcReg.two, HasDstReg.no, HFuType.bru, BranchType.bltu),
    BGEU -> List(NumSrcReg.two, HasDstReg.no, HFuType.bru, BranchType.bgeu),
