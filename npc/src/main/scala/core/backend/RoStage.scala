...
object ReadOpStage {
  class MicroOpWithData(implicit p: Parameters) extends MicroOp {
    val srcs = Vec(srcDataNum, UInt(XLEN.W))
  }
  class IOBundle(implicit p: Parameters) extends CoreBundle {
    val in       = Flipped(Decoupled(new MicroOp))
    val out      = Decoupled(new MicroOpWithData())
    val redirect = Redirect.input()
    val fromPrf  = Vec(srcRegNum, Input(UInt(XLEN.W)))
  }
}

class ReadOpStage(fuType: FuType.Type)(implicit p: Parameters) extends CoreDelegate {

  val io = Wire(new ReadOpStage.IOBundle())
  io.suggestName(s"${fuType}_ReadOpStage_io")
  def genIO = {
    val ret = IO(io.cloneType)
    ret <> io
    ret
  }

  Connect.byType(io.out.bits, io.in.bits)
  io.out.bits.srcs(0) := LookupEnumDefault(io.in.bits.src1From, 0.U)(
    Seq(
      Src1From.reg -> io.fromPrf(0),
      Src1From.pc -> io.in.bits.pc
    )
  )

  io.out.bits.srcs(1) := LookupEnumDefault(io.in.bits.src2From, 0.U)(
    Seq(
      Src2From.reg -> io.fromPrf(0),
      Src2From.shamt -> io.in.bits.imm(5, 0),
      Src2From.imm -> io.in.bits.imm
    )
  )
  ...
  /* TODO:
   * 1. bypass from alu and mem, select data
   * 2. wake up RS
   * 3. alu can write srat at here, bacuase must be write back
   * 4. add some data different funtion units
   */

}

// class RoStage(fuKind: FuType.t)(implicit p: Parameters) extends CoreModule {
...
//   outBits.uOp          := inOrigin.uOp
//   outBits.destPregAddr := inBasic.destPregAddr
//   outBits.destAregAddr := inBasic.destAregAddr
//   outBits.exDetect     := inBasic.exDetect
//   outBits.robIndex     := inBasic.robIndex

//   if (EnableDebug) outBits.debugHW.get := inBasic.debugHW.get
...
//       outBranch.realBtbType := realBtbType
//       outBranch.realTarget  := Mux(isJ, jDest, Mux(isJr, jrDest, bDest))
//       outBranch.predict     := maExtraIn.predictResult
//       outBranch.pcVal       := maExtraIn.pcVal
//       //srcs select
//       when(AluType.useImm(aluType)) {
//         asg(outSrcs(1), Mux(AluType.zeroExt(aluType), ZeroExt(imm, 32), SignExt(imm, 32)))
//       }
...
//     outMem.idxMiss         := addrL12sb(11, DcacheOffsetWidth) =/= toCacheIdx
...
//   wakeUpSource.bits  := outBits.destPregAddr
