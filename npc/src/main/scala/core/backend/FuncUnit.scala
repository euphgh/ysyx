...
class FuncUnit(kind: FuType.t)(implicit p: Parameters) {
  ...
  io.roOutFire            := roStage.io.out.fire
  roStage.io.datasFromPrf := io.datasFromPrf
  ...
  if (FuType.needBpIn(kind)) {
    ...
    if (FuType.needOBpIn(kind)) {
      ...
      (0 until FuType.oBpNum(kind)).map(i => roBpIn(i) := outFuBp(i))
    }
    //last channel from self fu
    if (FuType.needSBpIn(kind)) {
      val channel = FuType.bpNum(kind) - 1
      roBpIn(channel).bits  := io.out.bits.wPrf
      roBpIn(channel).valid := io.out.valid
    }
  }
}
