...
abstract class Divider(implicit p: Parameters) extends CoreModule {
  val io = IO(new Bundle {
    val in = Flipped(Valid(new Bundle {
      val srcs   = Vec(srcRegNum, Output(UInt(dataWidth.W)))
  ...
  def working = {
    ...
    quot := (quot << 2)(31, 0) | Cat(0.U(30.W), Cat(!sub3(64) || !sub2(64), !sub3(64) || (sub2(64) && !sub1(64))))
    val wrap = cnt.andR
    cnt   := cnt + 1.U
...
object Divider {
  def apply(useShift: Boolean = false)(implicit p: Parameters) = {
    if (useShift) new ShiftDivider
    else new DoubleDivider
  }

  class DoubleDivider(implicit p: Parameters) extends Divider {
    switch(state) {
  ...
  class ShiftDivider(implicit p: Parameters) extends Divider {
    val shiftWidth = 8
