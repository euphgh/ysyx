...
class SimRegfile(
  ...
  implicit p: Parameters)
    extends BaseRegfile(numReadPorts, numWritePorts, hasZero, numEntries, dataWidth) {
...
class SynRegfile(
  ...
  regfile.zipWithIndex.foreach {
    case (data_banks, bank_i) =>
      data_banks.zipWithIndex.foreach {
        case (bank, data_i) =>
          bank.io.writePorts.zip(io.writePorts).foreach {
            case (bank_port, port) =>
          ...
          bank.io.readPorts.zip(io.readPorts).foreach {
            case (bank_port, port) =>
              bank_port.addr := port.addr
          }
          bank.io.debug_ports.zip(io.debug_ports).foreach {
            case (bank_port, port) =>
              bank_port.addr := port.addr
          }
      }
...
object Regfile {
  def fastSim()(implicit p: Parameters) = !p(DebugOptionsKey).FPGAPlatform
  def apply(
    numReadPorts:  Int,
    numWritePorts: Int,
    hasZero:       Boolean,
  ...
  def apply(
    ...
    val regfile = Regfile(numReadPorts, numWritePorts, hasZero, numEntries, dataBits)
    val rdata = regfile.io.readPorts.zip(raddr).map {
      case (rport, addr) =>
        rport.addr := addr
        rport.data
    }
    regfile.io.writePorts.zip(wen).zip(waddr).zip(wdata).foreach {
    ...
    val debug_rdata = regfile.io.debug_ports.zip(debugRead.getOrElse(Seq())).map {
      case (port, addr) =>
        port.addr := addr
        port.data
    }
