...
class BChannel(val idLen: Int = 4)(implicit p: Parameters) extends CoreBundle {
  val id = UInt(idLen.W)
}

class DramReadIO(implicit p: Parameters) extends CoreBundle {
  val ar = Decoupled(new ARChannel)
  val r  = Flipped(Decoupled(new RChannel))
}
class DramWriteIO(implicit p: Parameters) extends CoreBundle {
  val aw = Decoupled(new AWChannel)
  val w  = Decoupled(new WChannel)
  val b  = Flipped(Decoupled(new BChannel))
}

class DramIO(implicit p: Parameters) extends CoreBundle {
  val ar = Decoupled(new ARChannel)
  val r  = Flipped(Decoupled(new RChannel))
  val aw = Decoupled(new AWChannel)
  val w  = Decoupled(new WChannel)
