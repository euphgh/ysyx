...
class BtbUpdateIO(implicit p: Parameters) extends CoreBundle {
...
class PhtUpdateIO(implicit p: Parameters) extends CoreBundle {
  val tagIdx   = Input(UInt((32 - log2Ceil(IcachLineBytes)).W))
  val instrOff = Vec(4, Input(UInt(instrOffWidth.W)))
...
class LocHisTab(implicit p: Parameters) extends CoreModule {
  import LocHisTab._
...
class BasicBPU[T <: Data](
  val gen:      T,
  val idxWidth: Int     = basicBpuIdxWidth,
  useRegs:      Boolean = true
)(
  implicit p: Parameters)
    extends CoreModule {
  val update = IO(new Bundle {
...
/**
  ...
  */
class BranchTargetBuffer(implicit p: Parameters) extends BasicBPU(new BtbOutIO(), basicBpuIdxWidth) {
...
/**
  ...
  */
class PatternHistoryTable(implicit p: Parameters) extends BasicBPU(UInt(2.W), basicBpuIdxWidth) {
...
class BCache(implicit p: Parameters) extends CoreModule {
  val io = IO(new Bundle {
