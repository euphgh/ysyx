...
static void init_readelf(const char *filename) {
  ...
  fread_at(fp, head.e_shoff + head.e_shentsize * head.e_shstrndx,
           sizeof(Elf_Shdr), &shstr_shd);
...
void load_elf_info(const char *filename,
                   void (*addEle)(int, int, const char *)) {
  ...
  char *funcTempName = malloc(512);
  for (int i = 0; i < total; i++) {
    Elf_Sym sym;
    int res = fread_at(fp, symbol_tb_a + symbol_shd.sh_entsize * i,
                       symbol_shd.sh_entsize, &sym);
    assert(res);
    if (ELF32_ST_TYPE(sym.st_info) == STT_FUNC) {
      Elf_Addr value = sym.st_value;
      uint64_t size = sym.st_size;
      get_sym_name(sym, funcTempName);
      addEle(value, size, strdup(funcTempName));
    }
  }
  free(funcTempName);
  fclose(fp);
