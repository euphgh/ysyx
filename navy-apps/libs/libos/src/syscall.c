...
int _gettimeofday(struct timeval *tv, struct timezone *tz) {
  return _syscall_(SYS_gettimeofday, (uintptr_t)&tv->tv_sec,
                   (uintptr_t)&tv->tv_usec, 0);
}

int _execve(const char *fname, char * const argv[], char *const envp[]) {
  _syscall_(SYS_execve, (uintptr_t)fname, (uintptr_t)argv, (uintptr_t)envp);
  return 0;
}
